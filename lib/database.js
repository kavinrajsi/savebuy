...
router.get('/create-customer-transaction', (req, res) => {
	let sql =
		'CREATE TABLE IF NOT EXISTS `savebuy`.`customer_transaction` ( `id` INT NOT NULL AUTO_INCREMENT , `receiptnumber` VARCHAR(255) NOT NULL , `groupnumber` VARCHAR(255) NOT NULL , `receiptdate` DATE NOT NULL , `formonth` VARCHAR(10) NOT NULL , `paidtype` VARCHAR(255) NOT NULL , `cashier` VARCHAR(255) NOT NULL , `term` INT(12) NOT NULL , `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`id`),  UNIQUE (`receiptnumber`, `groupnumber`), INDEX(`id`, `receiptnumber`, `groupnumber`, `receiptdate`, `paidtype`)) ENGINE = InnoDB';
	pool.query(sql, (err, result) => {
		if (err) throw err;
		console.log(result);
		res.render('404', {
